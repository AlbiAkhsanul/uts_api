<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Update Mitra</h2>
    </x-slot>
            ...
            <div class="max-w-4xl mx-auto px-6 space-y-6">
                <form action="{{ route('partners.update', $partner['id']) }}" method="POST" class="space-y-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="nama_partner" value="{{ old('nama_partner', $partner['nama_partner']) }}" class="w-full border rounded p-2" required>
                    </div>

                    <div>
                        <label for="negara_asal">Negara Asal Mitra (Tulis Kode Negara Saja : contoh SG = Singapore)</label>
                        <input type="text" id="negara_asal" name="negara_asal" value="{{ old('negara_asal', $partner['negara_asal']) }}" class="w-full border rounded p-2" required>
                    </div>
                    <!-- Preview bendera -->
                    <div class="mt-2">
                        <img id="flag-preview" src="https://flagsapi.com/{{ strtoupper($partner['negara_asal']) }}/shiny/64.png" alt="Flag preview" class="h-10" style="display: inline;">
                    </div>

                    <div>
                        <label for="no_telepon">Nomor Telepon Mitra</label>
                        <input type="number" name="no_telepon" value="{{ old('no_telepon', $partner['no_telepon']) }}" class="w-full border rounded p-2" required>
                    </div>

                    <div>
                        <label for="email">Email Mitra</label>
                        <input type="text" name="email" value="{{ old('email', $partner['email']) }}" class="w-full border rounded p-2" required>
                    </div>
